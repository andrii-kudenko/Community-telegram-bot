...
async def start_jobs_by_query(query: CallbackQuery, state: FSMContext):
    ...
    await query.message.edit_text("Hi there! Choose the action:", reply_markup=nav.jobsReplyChoiceMenu.as_markup())
...
async def handle_job_check_applicants_by_query(query: CallbackQuery, callback_data: nav.JobsCallback, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        if job_applicants:
            ...
            await handle_job_list_by_message(query.message, callback_data, state)       
...
async def handle_job_field_edit_by_query(query: CallbackQuery, callback_data: nav.JobsCallback, state: FSMContext):
    ...
    match callback_data.action:
        case "title":
            ...
            await query.message.edit_text("Provide new title")
        case "description":
        ...
        case "skills":
            ...
            await query.message.edit_text("Provide new skills")
        case "city":
            await state.set_state(JobEdit.location)
            await query.message.edit_text("Provide new city")
        case "address":
            await state.set_state(JobEdit.address)
            await query.message.edit_text("Provide new address")
